...
import { getMyConversations, getConversationById, getOrCreateConversation } from '@/services/conversationService';
import { getConversationMessages } from '@/services/messageService';
import Link from 'next/link';
import ChatBox from './ChatBox';
import styles from './messages.module.css';

export default async function MessagesPage({ searchParams }: { searchParams: Promise<{ conversationId?: string, propertyId?: string, tenantId?: string }> }) {
  ...
  if (resolvedSearchParams.propertyId) {
    ...
    if (user.role === 'LANDLORD') {
      if (!resolvedSearchParams.tenantId) {
        // If a landlord clicks a link without a tenantId, we can't create a conversation
        return <div className={styles.container}><p>Error: Tenant ID is required to start a conversation.</p></div>;
      }
      tId = resolvedSearchParams.tenantId;
    }
    
    let convId = null;
    let errorMsg = null;
    try {
      const conv = await getOrCreateConversation(tId, resolvedSearchParams.propertyId);
      convId = conv.id;
    } catch (e: any) {
      ...
      return (
        <div className={styles.container}>
          <div className={styles.main} style={{ padding: '2rem' }}>
            <h2 style={{ color: 'var(--color-danger)' }}>Could not start conversation</h2>
            <p>{errorMsg}</p>
            <p style={{ marginTop: '1rem', color: 'var(--color-muted)' }}>
              Property ID: {resolvedSearchParams.propertyId} <br />
              Tenant ID: {tId}
            </p>
          </div>
        </div>
    ...
    if (convId) {
      redirect(`/${user.role.toLowerCase()}/messages?conversationId=${convId}`);
    }
  }

  const { conversations } = await getMyConversations(user.id, user.role as "TENANT" | "LANDLORD", 1, 50);

  const activeConversationId = resolvedSearchParams.conversationId;
  let messages: any[] = [];
            ...
            {conversations.map((conv) => {
              ...
              return (
                <li key={conv.id} className={`${styles.convItem} ${isActive ? styles.activeConv : ''}`}>
                  <Link href={`/${user.role.toLowerCase()}/messages?conversationId=${conv.id}`} className={styles.convLink}>
